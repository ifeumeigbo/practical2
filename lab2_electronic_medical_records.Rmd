...
note_id: Unique identifier for each medical note/transcript in the dataset (id variable).\
description: Brief succinct summary or title describing the medical case.\
medical_specialty: The medical specialty, department, or unit for the particular case/appointment (e.g., Neurology, Surgery, Radiology).\
sample_name: Name or type of the medical procedure or condition of the patient.\
transcription: Entire written text of the medical note or conversation.\
keywords: Relevant medical keywords or terms associated with the note.\
...
```{r}
#Insert code here. 
unigram_counts <- tokenized.data.unigram %>%
...
A general-purpose lemmatizer may not work well for medical data due to several reasons. First, medical terminology often has specialized morphology that is not found in general dictionaries. Second, widely used medical abbreviations and acronyms may not be recognized. Third, medical procedures have specific naming conventions. Fourth, Latin or Greek medical terms tend to follow different linguistic rules. Finally, drug names and anatomical terms have unique patterns.
...
The plot demonstrates that Surgery and Orthopedic have more similar lemma frequencies, i.e. points closer to the diagonal line, compared to Radiology and Orthopedic. This makes sense because both Surgery and Orthopedic involve surgical procedures, sharing terminology related to operations, anatomy, and surgical techniques. In contrast, Radiology focuses on imaging and diagnosis, using a different vocabulary.
...
lemma.freq.surgery <- lemmatized.data %>%
  ...
  pivot_longer(`Orthopedic`:`Radiology`, names_to = "medical_specialty", values_to = "proportion")

ggplot(lemma.freq.surgery, aes(x=proportion, y=`Surgery`, 
  ...
  labs(y="Surgery", x = NULL,
       title="Lemma Frequency Comparison: Surgery vs Other Specialties")
...
A few lemmas stand out as specialty-specific:\
Orthopedic: 'range motion', 'carpal ligament', 'steri strips' - these are surgical/anatomical terms.\
Surgery: 'prepped draped', 'preoperative diagnosis' - these surgical procedure terminology.\
Radiology: 'carotid artery', 'heart rate' - these are imaging and cardiovascular terms.\
These reflect the specialized vocabulary of each medical field represented.
...
```{r}
analysis.data %>% dplyr::select(medical_specialty, transcription) %>% dplyr::filter(stringr::str_detect(transcription, 'carotid artery')) %>% dplyr::slice(1)
```
...
```{r}
# Fitting the 6-topic LDA model
emr.lda.6 <- LDA(emr.dcm, k=6, control=list(seed=42))
emr.topics.6 <- tidy(emr.lda.6, matrix='beta')

# Extracting top terms for 6-topic model
top.terms.6 <- emr.topics.6 %>% 
  group_by(topic) %>%
  slice_max(beta, n=10) %>%
  ungroup() %>%
  arrange(topic, -beta)
...
top.terms.6 %>%
  ...
  scale_y_reordered() +
  labs(title="Top Terms by Topic (6-Topic LDA)")
```

```{r}
# Analyzing the 6-topic assignments by specialty
specialty_gamma.6 <- tidy(emr.lda.6, matrix='gamma')
specialty_gamma.6 <- left_join(specialty_gamma.6, note_id_specialty_mapping)
...
specialty_gamma.6 %>%
  ...
  labs(x = "topic", y = expression(gamma),
       title="Topic Assignments by Medical Specialty (6-Topic LDA)")
```

While many terms from the five-topic model are present in the six-topic model, the additional topic enables more nuanced separation. This allows for the specialization of specialties into more specific sub-topics, such as different types of surgical procedures or imaging studies.
